...
static int fts_read_touchdata(struct fts_ts_data *data)
{
...
#if (FTS_DEBUG_EN && (FTS_DEBUG_LEVEL == 2))
	{
		...
		count += sprintf(g_sz_debug, "%02X,%02X,%02X", buf[0], buf[1],
				 buf[2]);
		for (i = 0; i < len; i++) {
			count += sprintf(g_sz_debug + count, ",%02X",
					 buf[i + 3]);
		}
	...
	for (i = 0; i < event->touch_point; i++) {
		if (event->au8_touch_event[i] == FTS_TOUCH_DOWN ||
			...
			if (event->area[i] > 0) {
				...
				FTS_ERROR("[A]Illegal touch-major: %d",
					  event->area[i]);
				input_report_abs(data->input_dev,
						 ABS_MT_TOUCH_MAJOR, 1); //0x05
			}
...
static int fts_gpio_configure(struct fts_ts_data *data)
{
	...
	FTS_FUNC_ENTER();
	/* request irq gpio */
	printk("%s reset_gpio: %d\n", __func__, data->pdata->reset_gpio);
	printk("check reset_gpio: %d\n",
	       gpio_is_valid(data->pdata->reset_gpio));
	...
	if (gpio_is_valid(data->pdata->irq_gpio)) {
		...
		data->client->irq = gpio_to_irq(data->pdata->irq_gpio);
		printk("client->irq: %d\n", data->client->irq);
	}
	/* request reset gpio */
