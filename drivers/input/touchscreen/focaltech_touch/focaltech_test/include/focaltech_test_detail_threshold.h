...
struct stCfg_MCap_DetailThreshold {
	unsigned char (
		*InvalidNode)[RX_NUM_MAX]; // Invalid node, the node is not tested
	unsigned char (*InvalidNode_SC)
		[RX_NUM_MAX]; //Invalid node, the SCAP node is not tested
	...
	int (*SCapRawDataTest_ON_Max)
		[RX_NUM_MAX]; //Starting from this is the Self Capacitance test item, the Mutual Capacitance of each test item must be put on the top.
	...
	int ForceTouch_SCapRawDataTest_ON_Max
		[FORCETOUCH_ROW]
		[RX_NUM_MAX]; //Starting from this is the force touch test item the Self Capacitance of each test item must be put on the top.
...
struct stCfg_SCap_DetailThreshold {
	...
	int DeltaCxTest_Sort
		[MAX_CHANNEL_NUM]; //Sort for 6x06 and 6x36 employed universally
	int DeltaCxTest_Area
		[MAX_CHANNEL_NUM]; //Area for 6x06 and 6x36 employed universally
