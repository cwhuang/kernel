...
int get_test_data(
	char *pTestData); //test result data. (pTestData, External application for memory, buff size >= 1024*80)
...
struct stTestItem {
	unsigned char
		ItemType; //Classification of test items   CfgItem, DataTestItem, GraphTestItem,
...
#elif ((FT8606_TEST) || (FT8607_TEST) || (FT8716_TEST) || (FT8736_TEST) ||     \
       (FTE716_TEST) || (FTE736_TEST) || (FT8006_TEST))
...
#define FTS_TEST_DBG(fmt, args...)                                             \
	do {                                                                   \
		printk(KERN_ERR "[FTS] [TEST]%s. line: %d.  " fmt "\n",        \
		       __FUNCTION__, __LINE__, ##args);                        \
	} while (0)
#define FTS_TEST_PRINT(fmt, args...) printk(KERN_ERR "" fmt, ##args)
#define FTS_TEST_FUNC_ENTER()                                                  \
	printk(KERN_ERR "[FTS][TEST]%s: Enter(%d)\n", __func__, __LINE__)
#define FTS_TEST_FUNC_EXIT()                                                   \
	printk(KERN_ERR "[FTS][TEST]%s: Exit(%d)\n", __func__, __LINE__)
#else
#define FTS_TEST_DBG(fmt, args...)                                             \
	do {                                                                   \
	} while (0)
#define FTS_TEST_PRINT(fmt, args...)                                           \
	do {                                                                   \
	} while (0)
#define FTS_TEST_FUNC_ENTER()
#define FTS_TEST_FUNC_EXIT()
