			     ...
			     unsigned char *readbuf, int readlen)
{
	...
	iret = fts_i2c_read(fts_i2c_client, writebuf, writelen, readbuf,
			    readlen);
#else
...
static int fts_test_entry(char *ini_file_name)
{
	...
	char *printdata = NULL;
	int iTestDataLen =
		0; //The actual length of the test data in the library is used to save the data to the file.
	int ret = 0;
	int icycle = 0, i = 0;
	int print_index = 0;
	...
	for (i = 0; i < iTestDataLen; i++) {
		if (('\0' == testdata[i]) //Meet the end
		    || (icycle ==
			FTS_TEST_PRINT_SIZE -
				2) //Meet the print string length requirements
