...
static int fts_ctpm_get_vendor_id(struct i2c_client *client)
{
	...
	if (fts_updatefun_curr.app_valid == false) {
		for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
			...
			auc_i2c_write_buf[2] =
				(u8)((CONFIG_VENDOR_ID_ADDR - 1) >> 8);
			auc_i2c_write_buf[3] = (u8)(CONFIG_VENDOR_ID_ADDR - 1);
			i_ret = fts_i2c_read(client, auc_i2c_write_buf, 4,
					     vendorid, 2);
			if (i_ret < 0) {
				FTS_DEBUG(
					"[UPGRADE]: read flash : i_ret = %d!!",
					i_ret);
				continue;
			}
		...
		if ((vendorid[1] != FTS_VENDOR_1_ID) &&
			...
			FTS_ERROR(
				"[UPGRADE]: vendor id error : 0x%x, can not upgrade!!",
				vendorid[1]);
			return -EIO;
		}
