...
struct fts_Upgrade_Info fts_updateinfo[] = {
	{ 0x54, FTS_MAX_POINTS_10, AUTO_CLB_NONEED, 2, 2, 0x54, 0x2c, 20,
	  2000 }, //,"FT5x46"
	{ 0x55, FTS_MAX_POINTS_5, AUTO_CLB_NEED, 50, 30, 0x79, 0x03, 10,
	  2000 }, //,"FT5x06"
	{ 0x08, FTS_MAX_POINTS_5, AUTO_CLB_NEED, 50, 10, 0x79, 0x06, 100,
	  2000 }, //,"FT5606"
	{ 0x0a, FTS_MAX_POINTS_5, AUTO_CLB_NEED, 50, 30, 0x79, 0x07, 10,
	  1500 }, //,"FT5x16"
	{ 0x06, FTS_MAX_POINTS_2, AUTO_CLB_NONEED, 100, 30, 0x79, 0x08, 10,
	  2000 }, //,"FT6x06"
	{ 0x36, FTS_MAX_POINTS_2, AUTO_CLB_NONEED, 10, 10, 0x79, 0x18, 10,
	  2000 }, //,"FT6x36"
	{ 0x64, FTS_MAX_POINTS_2, AUTO_CLB_NONEED, 10, 10, 0x79, 0x1c, 10,
	  2000 }, //,"FT6336GU"
	{ 0x55, FTS_MAX_POINTS_5, AUTO_CLB_NEED, 50, 30, 0x79, 0x03, 10,
	  2000 }, //,"FT5x06i"
	{ 0x14, FTS_MAX_POINTS_5, AUTO_CLB_NONEED, 30, 30, 0x79, 0x11, 10,
	  2000 }, //,"FT5336"
	{ 0x13, FTS_MAX_POINTS_5, AUTO_CLB_NONEED, 30, 30, 0x79, 0x11, 10,
	  2000 }, //,"FT3316"
	{ 0x12, FTS_MAX_POINTS_5, AUTO_CLB_NONEED, 30, 30, 0x79, 0x11, 10,
	  2000 }, //,"FT5436i"
	{ 0x11, FTS_MAX_POINTS_5, AUTO_CLB_NONEED, 30, 30, 0x79, 0x11, 10,
	  2000 }, //,"FT5336i"
	{ 0x58, FTS_MAX_POINTS_5, AUTO_CLB_NONEED, 2, 2, 0x58, 0x2c, 20,
	  2000 }, //"FT5822"
	{ 0x59, FTS_MAX_POINTS_10, AUTO_CLB_NONEED, 30, 50, 0x79, 0x10, 1,
	  2000 }, //"FT5x26"
	{ 0x86, FTS_MAX_POINTS_10, AUTO_CLB_NONEED, 2, 2, 0x86, 0xA7, 20,
	  2000 }, //"FT8607"
	{ 0x87, FTS_MAX_POINTS_10, AUTO_CLB_NONEED, 2, 2, 0x87, 0xC6, 20,
	  2000 }, //"FT8736"
	{ 0x87, FTS_MAX_POINTS_10, AUTO_CLB_NONEED, 2, 2, 0x87, 0xA6, 20,
	  2000 }, //"FT8716"
	{ 0x0E, FTS_MAX_POINTS_2, AUTO_CLB_NONEED, 10, 10, 0x79, 0x18, 10,
	  2000 }, //,"FT3x07"
};
...
void fts_update_fw_ver(struct fts_ts_data *data)
{
	...
	dev_info(&client->dev, "Firmware version = %d.%d.%d", data->fw_ver[0],
...
void fts_ctpm_get_upgrade_array(void)
{
	...
	for (i = 0; i < 5; i++) {
		ret = fts_i2c_read_reg(fts_i2c_client, FTS_REG_CHIP_ID,
				       &chip_id);
		if (ret < 0) {
...
#if (FTS_CHIP_IDC)
	u8 i = 0;
	unsigned char inRomBoot =
		0x00; //0x01 : run in rom boot;  0x02 : run in pram boot
#endif
...
#if ((FTS_CHIP_TYPE == _FT8716) || (FTS_CHIP_TYPE == _FT8736) ||               \
     (FTS_CHIP_TYPE == _FT8607))
	if (fw_type == FW_SIZE) {
...
u8 fts_ctpm_check_fw_status(struct i2c_client *client)
{
	...
	FTS_DEBUG(
		...
		uc_tp_vendor_id, uc_chip_id, chip_types.chip_idh,
		FTS_VENDOR_1_ID);
...
bool fts_ctpm_check_need_upgrade(struct i2c_client *client)
{
	...
	if (fw_status ==
	    FTS_RUN_IN_APP) //call fts_flash_get_upgrade_info in probe function firstly.
	{
		...
		FTS_INFO(
			"[UPGRADE]: app is invalid, need upgrade, reset now!!");
...
{
	int i_ret = 0;
	i_ret = fts_updatefun_curr.upgrade_with_app_bin_file(
		client, FTS_UPGRADE_FW_APP_BIN);
	if (i_ret < 0) {
...
{
	int i_ret = 0;
	i_ret = fts_updatefun_curr.upgrade_with_lcd_cfg_bin_file(
		client, FTS_UPGRADE_LCD_CFG_BIN);
	if (i_ret < 0) {
	...
	if (bUpgradeFlag) {
		do {
			...
			} else /* upgrade fail */
			{
				...
				fts_ctpm_rom_or_pram_reset(
					client, (!(uc_upgrade_times - 1)));
			}
		} while ((i_ret != 0) &&
			 (uc_upgrade_times <
