...
#define QUECTEL_WWAN_VERSION                                                   \
	"Quectel_Linux&Android_QMI_WWAN_Driver_" VERSION_NUMBER
...
#if LINUX_VERSION_CODE >                                                       \
	KERNEL_VERSION(3, 13, 0) //8f84985fec10de64a6b4cdfea45f2b0ab8f07c78
...
static void bridge_mode_rx_fixup(sQmiWwanQmap *pQmapDev, struct net_device *net,
				 struct sk_buff *skb)
{
	uint bridge_mode = 0;
	unsigned char *bridge_mac;
...
static void qmap_wake_queue(sQmiWwanQmap *pQmapDev)
{
	...
	for (i = 0; i < pQmapDev->qmap_mode; i++) {
		...
		if (qmap_net && netif_carrier_ok(qmap_net) &&
		    netif_queue_stopped(qmap_net)) {
			netif_wake_queue(qmap_net);
		}
	}
}
...
_rmnet_vnd_get_stats64(struct net_device *net, struct rtnl_link_stats64 *stats)
{
	struct qmap_priv *dev = netdev_priv(net);
	unsigned int start;
	int cpu;
	...
	if (nss_cb &&
	    dev->use_qca_nss) { // rmnet_nss.c:rmnet_nss_tx() will update rx stats
		stats->rx_packets = 0;
		stats->rx_bytes = 0;
	}
...
#if (LINUX_VERSION_CODE >                                                      \
     KERNEL_VERSION(4, 10, 0)) //bc1f44709cf27fb2a5766cadafe7e2ad5e9cb221
static void rmnet_vnd_get_stats64(struct net_device *net,
...
#else
static struct rtnl_link_stats64 *
rmnet_vnd_get_stats64(struct net_device *net, struct rtnl_link_stats64 *stats)
{
	...
	if (priv->agg_skb) {
		...
		if (ready2send == 0) {
			priv->agg_skb = alloc_skb(
				ctx->ul_data_aggregation_max_size, GFP_ATOMIC);
			if (priv->agg_skb) {
				skb_reset_network_header(
					priv->agg_skb); //protocol da1a is buggy, dev wwan0
...
#ifdef QUECTEL_BRIDGE_MODE
		if (priv->bridge_mode &&
		    bridge_mode_tx_fixup(pNet, skb, priv->bridge_ipv4,
					 priv->bridge_mac) == NULL) {
			dev_kfree_skb_any(skb);
			return NETDEV_TX_OK;
		}
#endif
...
static int qmap_register_device(sQmiWwanQmap *pDev, u8 offset_id)
{
	...
	if (nss_cb && use_qca_nss) {
		...
		if (rc) {
			...
			rtnl_lock();
			netdev_rx_handler_register(qmap_net, qca_nss_rx_handler,
						   NULL);
			rtnl_unlock();
		}
	}
...
int qma_setting_store(struct device *dev, QMAP_SETTING *qmap_settings,
		      size_t size)
{
	struct net_device *netdev = to_net_dev(dev);
	struct usbnet *usbnetdev = netdev_priv(netdev);
	...
	if (pQmapDev->tx_ctx.ul_data_aggregation_max_datagrams == 1 &&
	    qmap_settings->ul_data_aggregation_max_datagrams > 1) {
		...
		dev_info(
			dev,
			"ul_data_aggregation_max_datagrams=%d, ul_data_aggregation_max_size=%d, dl_minimum_padding=%d\n",
			pQmapDev->tx_ctx.ul_data_aggregation_max_datagrams,
			pQmapDev->tx_ctx.ul_data_aggregation_max_size,
			pQmapDev->tx_ctx.dl_minimum_padding);
	}
...
static int qmap_ndo_do_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	...
	case 0x89F2: //SIOCDEVPRIVATE
		rc = copy_from_user(&qmap_settings, ifr->ifr_ifru.ifru_data,
				    sizeof(qmap_settings));
		if (!rc) {
			rc = qma_setting_store(&dev->dev, &qmap_settings,
					       sizeof(qmap_settings));
		}
		break;
	...
	case 0x89F3: //SIOCDEVPRIVATE
		if (pQmapDev->use_rmnet_usb) {
			...
			for (i = 0; i < pQmapDev->qmap_mode; i++) {
				struct net_device *qmap_net =
					pQmapDev->mpQmapNetDev[i];

				if (!qmap_net)
					break;

				strcpy(pQmapDev->rmnet_info.ifname[i],
				       qmap_net->name);
			}
			rc = copy_to_user(ifr->ifr_ifru.ifru_data,
					  &pQmapDev->rmnet_info,
					  sizeof(pQmapDev->rmnet_info));
		}
		break;
					 ...
					 struct sk_buff *skb, gfp_t flags)
{
...
#ifdef QUECTEL_BRIDGE_MODE
	{
		...
		if (pQmapDev->bridge_mode &&
		    bridge_mode_tx_fixup(dev->net, skb, pQmapDev->bridge_ipv4,
					 pQmapDev->bridge_mac) == NULL) {
			dev_kfree_skb_any(skb);
			return NULL;
		}
	}
...
#if defined(QUECTEL_WWAN_QMAP)
static struct sk_buff *qmap_qmi_wwan_tx_fixup(struct usbnet *dev,
					      struct sk_buff *skb, gfp_t flags)
{
	struct qmi_wwan_state *info = (void *)&dev->data;
	sQmiWwanQmap *pQmapDev = (sQmiWwanQmap *)info->unused;
...
static void qmap_packet_decode(sQmiWwanQmap *pQmapDev, struct sk_buff *skb_in,
			       struct sk_buff_head *skb_chain)
{
	...
	while (skb_in->len > sizeof(struct qmap_hdr)) {
		...
		{
			struct iphdr *ip4h =
				(struct iphdr *)(&skb_in->data[hdr_size]);
		...
		{
			struct ipv6hdr *ip6h =
				(struct ipv6hdr *)(&skb_in->data[hdr_size]);
...
#if (LINUX_VERSION_CODE >                                                      \
     KERNEL_VERSION(4, 10, 0)) //bc1f44709cf27fb2a5766cadafe7e2ad5e9cb221
static void (*_usbnet_get_stats64)(struct net_device *net,
				   struct rtnl_link_stats64 *stats);

static void qmi_wwan_get_stats64(struct net_device *net,
				 struct rtnl_link_stats64 *stats)
{
...
#else
static struct rtnl_link_stats64 *(*_usbnet_get_stats64)(
	struct net_device *net, struct rtnl_link_stats64 *stats);

static struct rtnl_link_stats64 *
qmi_wwan_get_stats64(struct net_device *net, struct rtnl_link_stats64 *stats)
{
...
static int qmi_wwan_register_subdriver(struct usbnet *dev)
{
	...
	/* register subdriver */
#if (LINUX_VERSION_CODE >                                                      \
     KERNEL_VERSION(5, 12, 0)) //cac6fb015f719104e60b1c68c15ca5b734f57b9c
...
static int qmi_wwan_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	if (dev->driver_info->flags & FLAG_NOARP) {
		...
		ret = usb_string(dev->udev, dev->udev->descriptor.iProduct, buf,
				 sizeof(buf));
		if (ret > 0) {
			buf[ret] = '\0';
		}
		...
		usb_control_msg(
			...
			1, //active CDC DTR
			intf->cur_altsetting->desc.bInterfaceNumber, NULL, 0,
			100);
	}
	...
	if (!status) {
		sQmiWwanQmap *pQmapDev = (sQmiWwanQmap *)kzalloc(
			sizeof(sQmiWwanQmap), GFP_KERNEL);
		...
		if (dev->driver_info->flags & FLAG_NOARP) {
			...
			int lte_a =
				(idProduct == 0x0306 || idProduct == 0x030B ||
				 idProduct == 0x0512 || idProduct == 0x0620 ||
				 idProduct == 0x0800 || idProduct == 0x0801);

			if (qmap_size > 4096 ||
			    dev->udev->speed >=
				    USB_SPEED_SUPER) { //if meet this requirements, must be LTE-A or 5G
				lte_a = 1;
			}
			...
			if (lte_a && pQmapDev->qmap_mode == 0) {
				...
				if (qmap_mode == 0)
					qmap_mode =
						1; //old quectel-CM only check sys/module/wwan0/parameters/qmap_mode
			}
			...
			if (pQmapDev->qmap_mode) {
				...
				if (pQmapDev->qmap_mode > 1)
					pQmapDev->use_rmnet_usb = 1;
					 ...
					 idProduct == 0x0801)
					pQmapDev->use_rmnet_usb =
						1; //benefit for ul data agg
				pQmapDev->rmnet_info.size = sizeof(RMNET_INFO);
				pQmapDev->rmnet_info.rx_urb_size =
					pQmapDev->qmap_size;
				pQmapDev->rmnet_info.ep_type =
					2; //DATA_EP_TYPE_HSUSB
				pQmapDev->rmnet_info.iface_id = 4;
				pQmapDev->rmnet_info.qmap_mode =
					pQmapDev->qmap_mode;
				pQmapDev->rmnet_info.qmap_version =
					pQmapDev->qmap_version;
				...
				if (pQmapDev->use_rmnet_usb) {
					...
					pQmapDev->driver_info.data =
						dev->driver_info->data;
					dev->driver_info =
						&pQmapDev->driver_info;
				}
			}
		}
...
#define qmi_wwan_raw_ip_info                                                   \
	...
	.bind = qmi_wwan_bind, .unbind = qmi_wwan_unbind,                      \
	.manage_power = qmi_wwan_manage_power,                                 \
...
static const struct usb_device_id products[] = {
	...
	{ QMI_FIXED_RAWIP_INTF(0x2C7C, 0x0125, 4,
			       mdm9x07) }, /* Quectel EC20 (MDM9X07)/EC25/EG25 */
	...
	{ QMI_FIXED_RAWIP_INTF(
		0x2C7C, 0x0700, 3,
		mdm9x07) }, /* Quectel BG95 (at+qcfgext="usbnet","rmnet") */
	{ QMI_FIXED_RAWIP_INTF(0x2C7C, 0x0306, 4,
			       mdm9x40) }, /* Quectel EG06/EP06/EM06 */
	{ QMI_FIXED_RAWIP_INTF(0x2C7C, 0x030B, 4,
			       mdm9x40) }, /* Quectel EG065k/EG060K */
	{ QMI_FIXED_RAWIP_INTF(0x2C7C, 0x0512, 4,
			       mdm9x40) }, /* Quectel EG12/EP12/EM12/EG16/EG18 */
			       ...
			       const struct usb_device_id *prod)
{
	...
	if (!status) {
		...
		tasklet_init(&pQmapDev->txq, rmnet_usb_tx_wake_queue,
			     (unsigned long)pQmapDev);
		...
		if (pQmapDev->use_rmnet_usb) {
			...
			netdev_rx_handler_register(dev->net,
...
static int __init qmi_wwan_driver_init(void)
{
	...
	if (!nss_cb) {
		printk(KERN_ERR
		       "qmi_wwan_driver_init: driver load must after '/etc/modules.d/42-rmnet-nss'\n");
	}
#endif
